...
GAME_NOTATION_DIR = PROJECT_ROOT / "semantic_labeled_images" / "game_notation"
GAME_NOTATION_DIR.mkdir(parents=True, exist_ok=True)
...
def save_game_notation_image(image_id: int, image_path: str) -> None:
    src = Path(image_path)
    out_path = GAME_NOTATION_DIR / src.name

    img = cv2.imread(str(src))
    if img is not None:
        cv2.imwrite(str(out_path), img)
        print(f"[SAVED] Game notation image: {out_path}")
    ...
    for col_idx, col_img in enumerate(columns):
        ...
        for line_idx, line_img in enumerate(line_imgs):
            ...
            if is_notation_block(text):
                ...
                s = s1 + (s2 * 5)
                print(f"SCORE     {col_idx}-{line_idx:02d}: total={s} notation={s1} chess={s2}")
...
def process_single_image(image_id: int, image_path: str) -> None:
    ...
    temp_dir = PROJECT_ROOT / "temp" / f"notation_debug_{image_id}"
    temp_dir.mkdir(parents=True, exist_ok=True)
    ...
    if full_text.strip():
        save_game_notation_image(image_id, image_path)
...
def score_with_chess_engine(text: str) -> int:
    ...
    tokens = re.findall(
        r"\d+\.\.\.|\d+\.|O-O-O|O-O|[KQRBN]?[a-h]?[x]?[a-h][1-8](?:=[QRBN])?[+#]?|[KQRBN][a-h]?[1-8]?[+#]?",
        text
    )

    legal_count = 0
    consecutive_legal = 0
    max_consecutive_legal = 0
    ...
    for tok in tokens:
        ...
        try:
            ...
            legal_count += 1
            consecutive_legal += 1
            max_consecutive_legal = max(max_consecutive_legal, consecutive_legal)

        except Exception:
            consecutive_legal = 0
            continue

    return legal_count + max_consecutive_legal

# ---------------------------
# UNKNOWN IMAGES BATCH INPUT
# ---------------------------
def process_unknown_images_from_semantic_output() -> None:
    input_dir = PROJECT_ROOT / "semantic_labeled_images"

    if not input_dir.exists():
        print(f"[ERROR] Klasör bulunamadı: {input_dir}")
        return

    unknown_images = sorted(input_dir.glob("*_unknown_*.png"))

    print(f"[INFO] OCR'a gönderilecek unknown görsel sayısı: {len(unknown_images)}")

    if not unknown_images:
        print("[INFO] Unknown görsel bulunamadı.")
        return

    for image_path in unknown_images:
        try:
            image_id = int(image_path.name.split("_")[0])

            print("\n==============================")
            print(f"[UNKNOWN INPUT] image_id={image_id}")
            print(f"[IMAGE PATH] {image_path}")

            process_single_image(image_id, str(image_path))

        except Exception as e:
            print(f"[ERROR] {image_path.name} işlenemedi: {e}")


if __name__ == "__main__":
    process_unknown_images_from_semantic_output()