...
def calc_ssim(img1, img2):
    ...
    try:
        ...
        s, _ = ssim(i1_resized, i2_resized, full=True, win_size=win_size)
...
def calc_phash(img1, img2):
    ...
    return 1 - (h1 - h2) / 64.0  # 64 bit hash farkını normalize et

def calc_orb(img1, img2):
    """ORB benzerliği [0,1]"""
    try:
        i1 = np.array(img1.convert("L"))     #grayscale formatı icin -> "L" modu = luminance (parlaklık)
        i2 = np.array(img2.convert("L"))
